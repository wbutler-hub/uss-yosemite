package za.co.wethinkcode; /**AUTHOR: NICHOLAS SEWARD
...
/**
 * za.co.wethinkcode.Turtle is a selfcontained class that will allow students to make
...
public class Turtle implements Runnable, ActionListener, MouseListener, MouseMotionListener, KeyListener, ComponentListener, MouseWheelListener
{
    ...
    private static void init()
    {
        ...
        window = new JFrame("za.co.wethinkcode.Turtle");
        icon = new ImageIcon();
        setupBuffering();
...
//        System.out.println(Arrays.toString(fontNames));
     ...
     * of the applet you need to place <code>za.co.wethinkcode.Turtle.startApplet(this);</code>.
     ...
     * You don't need to use this outside of the za.co.wethinkcode.Turtle.java file.
     *
            ...
            for(Method m : Class.forName(className).getDeclaredMethods())
            {
                if (m.getName().equals(methodName))
                {
                    ...
                    for(Class paramType : m.getParameterTypes())
                    {
                        ...
                        if (!paramType.getName().equals("double") && !paramType.getName().equals("java.lang.Double") && !paramType.getName().equals("za.co.wethinkcode.Turtle"))
                        {
                            works=false;
                            break;
                        }
        ...
        try
        {
            ...
            for(Method m : Class.forName(className).getDeclaredMethods())
            {
                if (m.getName().equals(methodName))
                {
                    ...
                    {
                        //System.out.println(paramType.getName());
                        if (!paramType.getName().equals("java.lang.String") && !paramType.getName().equals("za.co.wethinkcode.Turtle"))
                        {
                            works=false;
                            break;
                        }
    ...
    {
        //za.co.wethinkcode.Turtle bob = new za.co.wethinkcode.Turtle();
        /*for(int i=0;i<360;i++)
        {
            bob.forward(i*1.25);
            bob.left(90.25);
